...
def install_dependencies():
    ...
    # need this since Blender's python does not have these libraries.
    try:
        import scipy
    except ImportError:
        import subprocess
...
def get_optimal_z_stretch(a:float):
    path = "./data/z_stretch/z_stretch_lut.json"
    with open(path) as f:
        z_stretch_lut = json.load(f)

    if a in z_stretch_lut:
        return z_stretch_lut[a]
    else:
        return None
